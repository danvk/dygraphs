...
import { DygraphsPlugin, PluginLayoutEvent, PluginDrawEvent } from '../dygraph-types';
 ...
 */
class Axes implements DygraphsPlugin {
  xlabels_: HTMLDivElement[];
  ylabels_: HTMLDivElement[];
  ...
  layout(e: PluginLayoutEvent) {
    ...
    if (g.numAxes() == 2) {
      if (g.getOptionForAxis('drawAxis', 'y2')) {
        const w = g.getOptionForAxis('axisLabelWidth', 'y2') + 2 * g.getOptionForAxis('axisTickSize', 'y2');
  ...
  willDrawChart(e: PluginDrawEvent) {
    const g = e.dygraph;
    if (!g.getOptionForAxis('drawAxis', 'x') &&
    ...
    const makeDiv = function (txt: string, axis: string, prec_axis?: string) {
      /*
    ...
    const makeOptionGetter = function (axis: 'x' | 'y' | 'y2') {
      return (option: string) => g.getOptionForAxis(option, axis);
    };
      ...
      if (layout.yticks && layout.yticks.length > 0) {
        ...
        layout.yticks.forEach(tick => {
          if (tick.label === undefined)
            return; // this tick only has a grid line.
          let x = area.x;
          let sgn = 1;
          let prec_axis = 'y1';
          ...
          context.stroke();
          */
          const label = makeDiv(tick.label, 'y', num_axes == 2 ? prec_axis : null);
    ...
    if (g.getOptionForAxis('drawAxis', 'x')) {
      if (layout.xticks) {
        ...
        layout.xticks.forEach(tick => {
          if (tick.label === undefined)
            return; // this tick only has a grid line.
          const x = area.x + tick.pos * area.w;
          const y = area.y + area.h;
          /* Tick marks are currently clipped, so don't bother drawing them.
          context.beginPath();
          context.moveTo(halfUp(x), halfDown(y));
          ...
          context.stroke();
          */
          const label = makeDiv(tick.label, 'x');
