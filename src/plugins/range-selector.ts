...
class RangeSelector implements DygraphsPlugin {
  ...
  fgcanvas_ctx_: CanvasRenderingContext2D;
  leftZoomHandle_: HTMLImageElement;
  rightZoomHandle_: HTMLImageElement;
  graphDiv_: HTMLDivElement;
  isChangingRange_: boolean;
  canvasRect_: Area;
  ...
  // Private methods
  //------------------------------------------------------------------
  getOption_(name: string, opt_series?: string) {
    return this.dygraph_.getOption(name, opt_series);
  }

  setDefaultOption_(name: string, value: any) {
   ...
   */
  reserveSpace_(e: PluginLayoutEvent) {
    if (this.getOption_('showRangeSelector')) {
   ...
   */
  resize_() {
    const setElementRect = (canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, rect: Area, pixelRatioOption: number) => {
  ...
  initInteraction_() {
    ...
    const onCanvasHover = function (e: MouseEvent) {
    ...
    const onZoomHandleTouchEvent = function (e: TouchEvent) {
    ...
    const onCanvasTouchEvent = function (e: TouchEvent) {
      if (e.type == 'touchstart' && e.targetTouches.length == 1) {
    ...
    const addTouchEvents = function (elem, fn) {
