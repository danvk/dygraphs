...
describe("rolling-average", function () {
    ...
    it("testRollingAverage", function () {
        ...
        g.setSelection(0);
        assert.deepEqual("0: Y: 0", Util.getLegend());
        g.setSelection(1);
        assert.deepEqual("1: Y: 1", Util.getLegend());
        g.setSelection(2);
        assert.deepEqual("2: Y: 2", Util.getLegend());
        g.setSelection(3);
        assert.deepEqual("3: Y: 3", Util.getLegend());
        assert.deepEqual(1, g.rollPeriod());

        g.updateOptions({rollPeriod: 2});
        g.setSelection(0);
        assert.deepEqual("0: Y: 0", Util.getLegend());
        g.setSelection(1);
        assert.deepEqual("1: Y: 0.5", Util.getLegend());
        g.setSelection(2);
        assert.deepEqual("2: Y: 1.5", Util.getLegend());
        g.setSelection(3);
        assert.deepEqual("3: Y: 2.5", Util.getLegend());
        assert.deepEqual(2, g.rollPeriod());
        ...
        assert.deepEqual("1: Y: 0.5", Util.getLegend());
        g.setSelection(2);
        assert.deepEqual("2: Y: 1", Util.getLegend());
        g.setSelection(3);
        assert.deepEqual("3: Y: 2", Util.getLegend());
        assert.deepEqual(3, g.rollPeriod());
        ...
        assert.deepEqual("2: Y: 1", Util.getLegend());
        g.setSelection(3);
        assert.deepEqual("3: Y: 1.5", Util.getLegend());
        assert.deepEqual(4, g.rollPeriod());
    });
    ...
    it("testRollBoxDoesntDisapper", function () {
        ...
        assert.deepEqual(1, roll_box.length);
        assert.deepEqual("1", roll_box[0].value);
    ...
    it("testRollErrorBars", function () {
        ...
        for (var i = 1; i < data.length; i++) {
            ...
            assert.deepEqual(value, rolled[i][1], "unexpected rolled average");
            assert.deepEqual(value - variance, rolled[i][2][0], "unexpected rolled min");
            assert.deepEqual(value + variance, rolled[i][2][1], "unexpected rolled max");
        }
    });
    ...
    it("testRollFractionsBars", function () {
        ...
        for (var i = 0; i < data.length; i++) {
            assert.deepEqual(values[i], Math.round(rolled[i][1]), "unexpected rolled average");
            assert.deepEqual(lows[i], Math.round(rolled[i][2][0]), "unexpected rolled min");
            assert.deepEqual(highs[i], Math.round(rolled[i][2][1]), "unexpected rolled max");
        }
    });
