...
def _dovar(vars, match, e):
    ...
    if cmd == b"set":
        if val is None:
            e("SSI set(%s) without value" % var)
            return _ssierr
        vars[var] = val
        return _ssiok
    elif cmd == b"echo":
        if val is not None:
            e("SSI get(%s) with value" % var)
        if not var in vars:
            e("SSI get(%s) without set" % var)
            return _ssierr
        return vars[var]
    else:
        e("SSI unknown command(%s)" % cmd)
    return _ssierr
...
def InlineIncludes(path, errorfn):
  ...
  content = re.sub(br'<!-- *#(set|echo) *var=[\'"]([^\'"]+)[\'"](?: *value=[\'"]([^\'"]+)[\'"])? *-->\s*',
      lambda x: _dovar(vars, x, errorfn),
      content)
  return content
