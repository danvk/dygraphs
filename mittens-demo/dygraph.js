...
Dygraph.prototype.generateLegendHTML_ = function(x, sel_points) {
  ...
  if (typeof(x) !== 'undefined') {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      var div = document.getElementById("y-label" + i);
      ...
      var dom_y = this.toDomYCoord(pt.yval);
      if (i == 0) prev_y = dom_y;
      if (i == 1 && prev_y && Math.abs(dom_y - prev_y) < 16) {
        if (dom_y < prev_y) dom_y = prev_y - 16;
        else if (dom_y > prev_y) dom_y = prev_y + 16;
      }
      div.style.display = 'block';
      div.style.position = 'absolute';
      div.style.top = (dom_y - 8) + 'px';
      div.style.width = '40px';
      div.style.height = '16px';
      div.style.left = Math.floor(this.plotter_.area.x - 43) + 'px';
      div.style.color = c;
      div.innerHTML = yval;
    }

    var dom_x = this.toDomXCoord(pt.xval);
    var div = document.getElementById("x-label");
    div.style.display = 'block';
    div.style.position = 'absolute';
    div.style.top = (this.plotter_.area.y + this.plotter_.area.h + 2) + 'px';
    div.style.left = (dom_x - 40) + 'px';
    div.style.width = '80px';
    div.innerHTML = this.attr_('xValueFormatter')(x, displayDigits);

    // HACK!
    document.styleSheets[0].cssRules[0].style.cssText = 'color: gray';
  } else {
    for (var i = 0; i < 2; i++) {
      var div = document.getElementById("y-label" + i).style.display = 'none';
    }
    document.getElementById("x-label").style.display = 'none';
    // HACK!
    document.styleSheets[0].cssRules[0].style.cssText = 'color: black';
  }
